...
serve-docs:
    #!/usr/bin/env -S julia {{ threads-arg + docs-project }}
    using Revise, LiveServer, {{ project-name }}
    servedocs(
        launch_browser=true;
...
format:
    #!/usr/bin/env -S julia
    using JuliaFormatter, {{ project-name }}
    format(
        {{ project-name }},
...
benchmark:
    #!/usr/bin/env -S julia {{ args }}
    using PkgBenchmark
    benchmarkpkg("{{ project-name }}")

# Tune the package benchmarks
tune-benchmark:
    #!/usr/bin/env -S julia {{ args }}
    using PkgBenchmark
    benchmarkpkg("{{ project-name }}", retune=true)

# Profile package imports
profile-imports:
    #!/usr/bin/env -S julia
    using InteractiveUtils
    @time_imports using {{ project-name }}

# Run the package's build
build:
    #!/usr/bin/env -S julia {{ args }}
    using Pkg
    Pkg.build({{ project-name }})
...
compile:
    #!/usr/bin/env -S julia {{ threads-arg + project-arg }}
    import Pkg
